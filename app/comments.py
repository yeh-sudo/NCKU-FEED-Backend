...
comments_args.add_argument("id", type=str, help="comment's id")
...
class Comments(Resource):
    ...
    def post(self):
        ...
        result = self.database_processor.insert_comment(comment.dict())
        if result != "False":
            response = comment.dict()
            response["_id"] = result
            return response, 201
        else:
            return {}, 500
    ...
    def delete(self):
        ...
        args = comments_args.parse_args()
        if self.database_processor.delete_comment(args.id):
            return {}, 200
        else:
            return {}, 500
